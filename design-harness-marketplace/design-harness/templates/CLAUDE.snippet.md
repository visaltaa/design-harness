...
- `@.claude/rules/design-system.md` — this project's evolving rules **and** test
  suite. Enforced by `/design-check`, grown by `/approve`.
...
1. **Sense** — when new work appears, the `design-context` skill grounds it
   against the rules and asks clarifying questions. Optionally `/explore` to
   fan out variants, then reconverge on one.
2. **Build** — do the work on a dedicated git worktree/branch, never on the base.
3. **Prove** — run `/design-check`: it starts the dev server, captures a
...
4. **Decide** — you correct the work, or run `/approve`, which merges the branch,
...
- Never merge design work without a passing proof. (A hook enforces this on
  `git merge`.)
- The `design-check report` format is fixed — see `/design-check`. It must always
