...
build and `/design-check` will be measured against.

## 5. Offer open-thinking (don't force it)
If the direction is non-obvious or has real trade-offs, offer:
> "Want me to fan out a few variants first? Run **/explore <this work>**."
...
`/approve` folds it in (acceptance criteria graduate into `DS-*` checks) — never
...
- Don't write to `design-system.md` or `visual-baseline.md` — only `/approve` does.
